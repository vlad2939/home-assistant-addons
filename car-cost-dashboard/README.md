# Evidență Cheltuieli Auto — Home Assistant Add-on

Aplicație web locală pentru urmărirea riguroasă a tuturor costurilor unui Hyundai i20, rulată ca add-on nativ în Home Assistant. Datele sunt stocate permanent în volumul persistent al add-on-ului, fără baze de date externe, fără conturi cloud, fără dependențe de internet. Serverul Node.js inclus servește interfața și gestionează citirea și scrierea datelor. Aplicația rulează complet offline și este accesibilă direct din bara laterală a Home Assistant, cu suport Ingress.

---

## Instalare ca repository de third-party add-on

### Pasul 1 — Adaugă repository-ul în Home Assistant

În Home Assistant, navighează la:

**Settings → Add-ons → Add-on Store**

Apasă meniul cu trei puncte (⋮) din colțul din dreapta sus și alege **Repositories**. Adaugă URL-ul:

```
https://github.com/vlad2939/car-cost-dashboard
```

Apasă **Add**, apoi **Close**.

### Pasul 2 — Instalează add-on-ul

După adăugarea repository-ului, apasă din nou meniul (⋮) și alege **Check for updates**. Add-on-ul **Car Cost Dashboard** va apărea în lista din Add-on Store. Apasă pe el, apoi **Install**.

### Pasul 3 — Pornire și acces

- Mergi la **Settings → Add-ons → Car Cost Dashboard**
- Apasă **Start**
- Activează **Show in sidebar** dacă vrei acces rapid din bara laterală
- Deschide interfața din tab-ul **Web UI** sau direct din sidebar

La prima pornire, add-on-ul copiază automat datele inițiale din imaginea Docker în zona persistentă `/data/car-cost-dashboard`. Modificările ulterioare sunt scrise exclusiv în această zonă, astfel că actualizările add-on-ului nu afectează datele existente.

---

## Actualizare add-on

Când există o versiune nouă în repository-ul GitHub, Home Assistant afișează notificarea de actualizare în pagina add-on-ului. Apasă **Update** — datele sunt păstrate intact în zona persistentă.
...
```text
car-cost-dashboard/
├── config.yaml             — manifest add-on (nume, versiune, porturi, Ingress)
├── repository.yaml         — metadata repository third-party
├── Dockerfile              — imagine pe baza de amd64-base, instalează Node.js
├── run.sh                  — entrypoint container, configurează DATA_DIR și PORT
├── server.js               — server HTTP, API REST, normalizare și persistență date
├── package.json            — configurare Node, script start
├── README.md               — această documentație (afișată în GitHub și Add-on Store)
├── DOCS.md                 — documentație scurtă afișată în interfața HA
├── CHANGELOG.md            — istoricul versiunilor
├── LICENSE                 — licența proiectului
├── HOME_ASSISTANT_ADDON_NOTES.md — note tehnice despre adaptarea server.js
├── public/
│   ├── index.html          — structura HTML a aplicației
│   ├── app.js              — logica clientului JavaScript
│   ├── styles.css          — tema Asphalt & Steel, layout, responsive
│   ├── readme.md           — documentația afișată în popup-ul din aplicație
│   └── hyundai-logo.svg    — sigla Hyundai folosită în sidebar
└── data/
    ├── expenses.json       — cheltuieli non-combustibil (date inițiale, copiate la prima pornire)
    ├── fuel.json           — alimentări (date inițiale, copiate la prima pornire)
    └── meta.json           — metadate mașină (date inițiale, copiate la prima pornire)
```
...
Add-on-ul folosește două locații pentru date:

- **`/app/data/`** — datele inițiale incluse în imaginea Docker (servesc drept seed la prima pornire).
- **`/data/car-cost-dashboard/`** — zona persistentă a add-on-ului în Home Assistant. Toate scrierile din aplicație merg exclusiv aici. Această locație supraviețuiește actualizărilor și repornirii containerului.

Variabila de mediu `DATA_DIR` este setată de `run.sh` la `/data/car-cost-dashboard` și consumată de `server.js` pentru a determina unde citește și scrie fișierele JSON.

---

## Tehnologii și arhitectură

Stiva tehnologică este intenționat minimalistă: Node.js nativ (fără framework), HTML/CSS/JS static, fișiere JSON ca bază de date. Nu există npm install la runtime, bundlere, transpilere sau dependențe de producție. Fonturile Barlow și Barlow Condensed sunt încărcate de la Google Fonts la prima accesare; ulterior, browserul le pune în cache.

- `server.js` — server HTTP scris cu modulele native `http`, `fs` și `path`. Expune API-ul REST și servește fișierele statice din `public`. Suportă `DATA_DIR` pentru persistență în HA și `PORT` configurabil.
...
| Port intern | Port extern (host) | Utilizare                   |
|-------------|-------------------|-----------------------------|
| 3000        | 3030              | Acces direct fără Ingress   |

Add-on-ul suportă **Ingress** — interfața este accesibilă direct din bara laterală a Home Assistant, fără a expune portul în rețea.
...
Butonul de comutare temă (sus dreapta, în topbar) salvează alegerea în `localStorage` sub cheia `carDashboardTheme`. La reîncărcare, tema este restaurată automat înainte ca pagina să fie randată.
...
- `Consum mediu` — consumul mediu ponderat în litri la 100 km (calculat diferit cu sau fără filtru de perioadă).
...
- `odometerKm` — kilometrajul odometrului la data cheltuielii. Opțional, null dacă nu e relevant.
...
`ensureDataFiles()` este apelat la pornire și creează directorul `DATA_DIR` și fișierele JSON lipsă. La prima pornire în HA, dacă fișierele lipsesc din zona persistentă, le copiază din `data/` inclus în imagine (datele inițiale). Dacă nici acelea nu există, creează fișiere goale cu valori implicite. Această funcție face add-on-ul rezistent la instalări noi.

`listenWithFallback(port)` înregistrează un listener pentru eroarea `EADDRINUSE` și încearcă recursiv portul următor. Oprește recursivitatea dacă variabila `PORT` a fost setată explicit — în acel caz, eșuează cu eroare.
...
Datele sunt scrise pe disc imediat după fiecare operație CRUD, fără buffer sau queue. `writeJson()` scrie întregul array la fiecare modificare — potrivit pentru volumul unei aplicații personale (sute de înregistrări, nu milioane). Înainte de scriere, creează automat directorul destinație dacă nu există (`fs.mkdirSync` cu `recursive: true`).
...
Backup-ul trebuie efectuat înainte de orice modificare majoră: adăugare de multe înregistrări, editare în masă sau actualizare a add-on-ului.
...
Strategia recomandată de backup: descarcă un backup după fiecare sesiune de introducere date, salvează pe PC (folderul de Downloads, Google Drive, stick USB). Fișierul de backup din ziua curentă poate fi recuperat complet în 30 de secunde via Restore.

> **Notă pentru utilizatorii Home Assistant:** backup-ul și restore-ul se fac exclusiv prin browser — descărcând și încărcând fișierul `.json` de pe PC-ul de pe care accesezi aplicația. Add-on-ul nu necesită Samba și nu scrie automat pe Windows.
...
Cel mai simplu mod: din aplicație, descarcă un backup JSON, editează câmpul `car` în fișierul descărcat, apoi restaurează backup-ul. Reîncarcă aplicația în browser — sidebar-ul afișează noul nume.
...
**Add-on-ul nu pornește** — verifică log-urile din **Settings → Add-ons → Car Cost Dashboard → Log**. Cel mai frecvent motiv este un conflict de port, dar în context HA portul este gestionat automat.

**Datele nu apar sau sunt goale** — verifică că directorul `/data/car-cost-dashboard/` există și conține fișierele JSON. La prima pornire, add-on-ul le creează automat; dacă ceva a mers greșit, repornește add-on-ul.

**Graficele nu apar** — graficele sunt desenate pe canvas după ce layout-ul este calculat. Dacă sunt complet goale, navighează în altă secțiune și înapoi la Dashboard, sau reîncarcă pagina.
...
**Ingress nu funcționează** — asigură-te că ai activat opțiunea din pagina add-on-ului. Accesul direct prin portul `3030` rămâne disponibil ca alternativă.
...
|---|---|
| Port intern | 3000 |
| Port extern (host) | 3030 |
| Date persistente (HA) | `/data/car-cost-dashboard/` |
| Date inițiale (imagine) | `/app/data/` |
| Variabilă port | `PORT` (setată de `run.sh`) |
| Variabilă date | `DATA_DIR` (setată de `run.sh`) |
| Temă salvată | `localStorage["carDashboardTheme"]` (`light` sau `dark`) |
| Format ID-uri | `prefix_timestamp_random` (ex: `expenses_1715688000000_ab12cd`) |
| Categorii cheltuieli | Acte, Piese + scule, Consumabile + diverse, Manopera |
| Format backup | JSON cu `version`, `exportedAt`, `expenses`, `fuel`, `meta` |
| Documentație popup | `public/readme.md` (link în footer-ul sidebar-ului) |
